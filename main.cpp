...
void optimalPage(int pg[], int pn, int fn)
{
    ...
    vector<int> *table = new vector<int>[fn + 1]{};
...
void pageFaults(int* ref, int len, int frame_size, int ctime)
{
    ...
    vector<int> *table = new vector<int>[frame_size + 1]{};
    ...
    for (int i = 0; i < frame_size; i++) {
        arr.push_back(make_pair(0, ctime));
    }
    ...
    for (int i = 0; i < len; i++) {
        ...
        // If page is not found
            ...
            {
                //cout << "page = " << page << endl;
                arr.insert(pos, make_pair(page, ctime));
                int j = 0;
                for (auto iter = arr.begin(); iter != arr.end(); iter++)
                    table[j++].push_back(iter->first);
            }    
